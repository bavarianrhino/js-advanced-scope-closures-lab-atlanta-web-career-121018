// produceDrivingRange() - Returns a function that then calculates whether a given trip is within range.
    // For example, produceDrivingRange(10) returns a function that will return false if the trip
    // is over 10 blocks distance and true if the distance is within range.
...
function produceDrivingRange(blockRange){
    return function (start, end){
        let range = (blockRange - Math.abs((parseInt(start) - parseInt(end))));
        return (range >= 0) ? `within range by ${range}` : `${Math.abs(range)} blocks out of range`
    };
}
